...
{synopt: {cmdab:a:bsorb(}{it:...}{cmd:,} {cmdab:save:fe)}}save fixed effects with autogenerated names {it:__hdfe*__}; useful when running predict afterwards{p_end}
...
{pstd}{cmd:hdfe} is a programmers' routine that serves as a building block to other regression packages so they can support multiple fixed effects (see for instance {search binscatter}, {browse "https://github.com/matthieugomez/stata-regife":regife} and {search poi2hdfe}).
