...
* Load dataset
	sysuse auto
	noi di as text " -  test predict after reghdfe+pweight"
	local lhs price
	local rhs mpg length gear
	local absvars foreign
	local weightvar turn
	fvunab tmp : `rhs'
...
* [TEST]
	...
	* 1. Run benchmark
	areg `lhs' `rhs' [pw=`weightvar'], absorb(`absvars')
	di e(df_a)
	TrimMatrix `K'
	...
	* 2. Run reghdfe and compare
	...
	reghdfe `lhs' `rhs' [pw=`weightvar'], ///
		absorb(FE=`absvars') vce(, suite(default)) keepsingletons
	TrimMatrix `K'
	...
	* 4) Test that the means of resid are zero , with #c. interactions
	...
	drop d xbd xb 
	reghdfe price weight [pw=`weightvar'], a(turn trunk##c.gear, save) tol(1e-12) maxiter(1e4) keepsingletons v(1)
	predict double resid, resid
	predict double d, d
	predict double xbd, xbd
	su d xbd
	...
	* 5) Check that the means match
	areg price weight ibn.trunk##c.gear [pw=`weightvar'], a(turn)
