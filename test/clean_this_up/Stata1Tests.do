...
if (`:list posof "OLS2" in TestSuite') {
	...
	sysuse auto, clear
	set varabbrev on
	rename price GgiUUX4erbzBp1OhWGr3Q3VacNaBhNU4
	rename weight c2HmOql9DT9NIV0Y70INLKTeMJc4Vb4x
	rename length KExviRvFlvzSj8oSKybpylfwzejp1Y7i
	
	rename foreign G2rLhbVUmOQvHUeECEQE5uBCUHz8BiQt	
	rename turn wrF2JnDC8suPiZcwey2J9gVip1H9son5
	rename rep SBem838aK6HaC6LC7V3haKreb1Z5XWJj
	set varabbrev on
	reghdfe  GgiUUX4erbzBp1OhWGr3Q3VacNaBhNU4 ///
		KExviRvFlvzSj8oSKybpylfwzejp1Y7i c2HmOql9DT9NIV0Y70INLKTeMJc4Vb4x ///
		, a(wrF2JnDC8suPiZcwey2J9gVip1H9son5#G2rLhbVUmOQvHUeECEQE5uBCUHz8BiQt) ///
		vce(cluster SBem838aK6HaC6LC7V3haKreb1Z5XWJj)
	ereturn list

	sysuse auto, clear
	reghdfe price length weight, a(turn#foreign) vce(cluster rep)
	set varabbrev off
	clear
...
* "within an obs"
	...
	* We can tell -hdfe- to avoid adjusting it
	reghdfe y x* , a(t) tol(1e-10) vce(cluster tt) dof_method(naive)
	test_one, cut
	noi di in ye  "  OK"
	
* -g- nests -i-
	reghdfe y x*, a(i t g)
	assert e(dof_method)=="bounds"
